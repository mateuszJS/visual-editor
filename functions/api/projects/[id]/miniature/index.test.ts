...
import {
  aliceProjectId,
  aliceSessionToken,
  aliceProjectIdWithoutMiniature,
  bobSessionToken,
} from '@/setup'
...
describe('GET /api/projects/[id]/miniature', () => {
  it('redirects if project exists, user is the owner, signed url generated with no errors', async () => {
    ...
    expect(response.headers.get('Location')).toBe(
      `https://storage-provider.com/signed-url?bucket=project-miniatures-preview&key=${aliceProjectId}&expiredsIn=604800`
    )
  })
  ...
  it('returns error if user is not signed in', async () => {
    const response = await onRequestGet(getContext(new Request('x:'), { id: aliceProjectId }))
...
describe('PUT /api/projects/[id]/miniature', () => {
  it('if everything is correct redirects to cloud provider url', async () => {
    const request = new Request('x:', {
      headers: {
        ...
        'content-length': '1024',
        'x-amz-meta-captured-at': '2025-01-01T00:00:00.001Z',
      },
      method: 'PUT',
      body: file,
    })
    const response = await onRequestPut(getContext(request, { id: aliceProjectId }))

    expect(response.status).toBe(307)
    const locationHeader = response.headers.get('Location')
    expect(locationHeader).toEqual(
      `https://storage-provider.com/signed-url?bucket=project-miniatures-preview&key=${aliceProjectId}&expiredsIn=300&contentLength=1024`
    )
  })
  ...
  it('returns error if user is not the owner of the project', async () => {
    const request = new Request('x:', {
      headers: {
        cookie: `session=${bobSessionToken}`,
        'content-length': '1024',
        'x-amz-meta-captured-at': '2025-01-01T00:00:00.001Z',
      },
      method: 'PUT',
  ...
  it('returns error if user is not signed in', async () => {
    const response = await onRequestPut(getContext(new Request('x:'), { id: aliceProjectId }))
  ...
  it('redirects if it is first upload of project id: 2 miniature', async () => {
    const request = new Request('x:', {
      headers: {
        cookie: `session=${aliceSessionToken}`,
        'content-length': '1024',
        'x-amz-meta-captured-at': '2025-01-01T00:00:00.000Z',
      },
      method: 'PUT',
      body: file,
    })
    const response = await onRequestPut(getContext(request, { id: aliceProjectIdWithoutMiniature }))

    expect(response.status).toBe(307)
    const locationHeader = response.headers.get('Location')
    expect(locationHeader).toEqual(
      `https://storage-provider.com/signed-url?bucket=project-miniatures-preview&key=${aliceProjectIdWithoutMiniature}&expiredsIn=300&contentLength=1024`
    )
  })

  it('returns error if provided request header x-amz-meta-captured-at contains older date than R2 object metadata captured-at', async () => {
    const request = new Request('x:', {
      headers: {
        cookie: `session=${aliceSessionToken}`,
        'content-length': '1024',
